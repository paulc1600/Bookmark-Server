...
class ThreadHTTPServer(ThreadingMixIn, http.server.HTTPServer):
    "This is an HTTPServer with thread-based concurrency."
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:
                # ----------------------------------------------------------------
                # (2) Valid short name -- use lookup as redirect
                # Send a 303 back to the long URI in memory[name].
                # ----------------------------------------------------------------
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("ERROR: One or both bookmark values not provided.".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("ERROR: Site '{}' cannot be accessed.".format(longuri).encode())
